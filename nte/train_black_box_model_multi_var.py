...
from nte.models import Linear, RNN , CNN
import json
import ssl
import os
import wandb
...
from nte.data.real.multivariate.AtrialFibrilation.AtrialFibrilation_MultiClass import AtrialFibrilation_MultiClass
...
def train_black_box_model_multi_class(dataset, hyper_params):
    ...
    with Tee(filename=f'{hyper_params["model_save_path"]}{hyper_params["model_name"]}.log'):
        ...
        if hyper_params['model'] == 'rnn':
            ...
            model = CNN(hyper_params["cnn_config"]["ninp"], hyper_params["num_features"], "Conv2d",hyper_params["cnn_config"]["nlayers"], hyper_params["cnn_config"]["nclasses"]).to(device)
        else:
        ...
        for epoch in range(hyper_params.num_epochs):
            ...
            for i, (X, y) in enumerate(train_loader):
                ...
                if hyper_params['model'] == 'rnn':
                    ...
                    X = torch.tensor(X.reshape(-1, 1, hyper_params["num_features"],int(hyper_params["timesteps"]/hyper_params["num_features"])), dtype=torch.float32)
                ...
                y_hat = model(X)
                optimizer.zero_grad()
                # y_hat =y_hat.reshape(-1,hyper_params["num_classes"])
        ...
        if hyper_params['model'] == 'rnn':
            ...
            y_hat = model(torch.tensor(dataset.test_data.reshape(-1, 1,hyper_params["num_features"], int(hyper_params["timesteps"]/hyper_params["num_features"])),dtype=torch.float32).to(device))
            y_hat_train = model(torch.tensor(dataset.train_data.reshape(-1, 1, hyper_params["num_features"],int(hyper_params["timesteps"]/hyper_params["num_features"])),dtype=torch.float32).to(device))
...
if __name__ == '__main__':
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    dataset =  AtrialFibrilation_MultiClass()
    ...
    print(f"Test -  Data: {dataset.test_data.shape} | Label: {dataset.test_label.shape}")
    nte_plots = 0
