...
class CMGradientSaliency(Saliency):
    ...
    def generate_saliency(self, data, label, **kwargs):
        ...
        #top class
        max_category = np.argmax(kwargs['target'].cpu().data.numpy())
        num_classes = kwargs['target'].cpu().data.numpy().shape[0]
        n_masks = [] * num_classes
        ...
        for c in range(num_classes):
            ...
            for i in tqdm.tqdm(range(self.args.max_itr)):
                ...
                if self.args.bbmg == 'yes':
                    timesteps = int(perturbated_input.shape[2]/self.args.window_size)
            ...
            n_masks.append(1-mask)
        return n_masks[max_category], self.perturbation_manager, n_masks
